    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or self.mouseButton == 1 then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                --new function that switches the tiles
                PlayState:switchTiles(self.highlightedTile, self.board.tiles[y][x])

                -- tween coordinates between the two so they swap
                Timer.tween(0.2, {
                    [self.oldTile] = {x = self.newPositionX, y = self.newPositionY},
                    [self.newTile] = {x = self.oldPositionX, y = self.oldPositionY}
                })
...
function PlayState:switchTiles(oldTile, newTile)
  self.oldTile = oldTile
  self.newTile = newTile

  -- save old tile x y
  local oldX = self.oldTile.gridX
  local oldY = self.oldTile.gridY

  self.oldPositionX = self.oldTile.x
  self.oldPositionY = self.oldTile.y

  -- save new tile x y
  local newX = self.newTile.gridX
  local newY = self.newTile.gridY

  self.newPositionX = self.newTile.x
  self.newPositionY = self.newTile.y

  self.oldTile.gridX = newX
  self.oldTile.gridY = newY
  self.newTile.gridX = oldX
  self.newTile.gridY = oldY

  -- swap tiles in the tiles table
  self.board.tiles[newY][newX] = self.oldTile

  self.board.tiles[oldY][oldX] = self.newTile
end

function PlayState:switchBack()
  self:swapTiles(self.tile2, self.tile1)

  Timer.tween(0.1, {
      [self.tile1] = {x = self.originalTile2X, y = self.originalTile2Y},
      [self.tile2] = {x = sself.originalTile1X, y = self.originaltile1Y}
  })
